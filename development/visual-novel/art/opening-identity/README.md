...
September 8, 2026. This records the opening-scene trial approved before the broader visual pass. The accepted composite is now selected for the game; the original input is frozen as `opening-original.png` so this comparison remains reproducible. See [the broader pass](../character-refinements/README.md) for subsequent character keys and propagation.
