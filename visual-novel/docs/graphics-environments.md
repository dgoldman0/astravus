...
[build_treehouse_states.py](../scripts/build_treehouse_states.py) assembles immutable original room/paper sources, the bounded later furnishing layer, and [relight_treehouse_states.py](../scripts/relight_treehouse_states.py)'s explicit lighting/weather states. It replaces the first near-identical variant producer as the current room-state workflow. [environment-edits.json](environment-edits.json) records actual source bytes, scripts, operations, output hashes and independent changed-pixel counts. Later furnished outputs also reference [treehouse-furnishings.json](treehouse-furnishings.json) and the generated-material register; retained material is limited to furniture and its vacated floor footprint.
