#!/usr/bin/env python3
"""Query the Lumen space register, verify its coverage, or render its wiki page.

Standard library only; no server, embeddings, uploads or persistent database.
The curated JSON is the source for both queries and the generated wiki page.
...
def source_link(src, root=ROOT):
    ...
    return f"[{src['path']}:{line}](../../{src['path']}#L{line})"
...
def render(data, root=ROOT):
    ...
    out.extend(["", "## Spaces and functions", "",
        f"{len(data['spaces'])} records, including provisions, groups of spaces, remote destinations and fictional settings. This is not a room count. All physical records currently remain **unplaced** in a complete reconstructed layout.", ""])
    ...
    for group in groups:
        ...
        for item in data["spaces"]:
            ...
            out.extend([f'<a id="{item["id"]}"></a>', f"#### {item['name']}", "",
                f"`{item['id']}` · Basis: **{item['basis']}** · Model: **{item['model_status']}**", "",
                ...
                "**Evidence:** " + "; ".join(source_link(src, root) for src in item["sources"]) + ".", ""])
    ...
    for rel in data["relations"]:
        ...
        out.append(f"| {links([rel['from_id'], rel['to_id']], lookup)} | {rel['kind']} | {rel['description']} {cites}. |")
...
def main():
    ...
    output.add_argument("--write", action="store_true", help="regenerate wiki page after verifying evidence")
    ...
    if args.check or args.write:
        ...
        if args.write:
            ...
            print("Generated wiki page differs; run --write after reviewing the register.", file=sys.stderr)
            return 1
